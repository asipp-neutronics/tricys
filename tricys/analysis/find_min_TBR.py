"""//TODO: 需要进一步完善和测试。本模块提供了一个用于查找最小氚增殖比（TBR）的图形用户界面。"""
...
def run_parameter_sweep(
    ...
    try:
        ...
        base_combined_filename = (
            f"{param_A_name.replace('.', '_')}_{param_B_name.replace('.', '_')}.csv"
        )
...
class SimulationUI:
    ...
    def __init__(self, root):
        ...
        self.package_path = os.path.join(self.project_root, self.config["package_path"])
        self.available_params = get_available_parameters(
            self.package_path, "FFCAS.Cycle"
        )
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        self.param_var = tk.StringVar(
            value=(
                "i_iss.T"
                if "i_iss.T" in self.available_params
                else (self.available_params[0] if self.available_params else "")
            )
        )
        ttk.Combobox(
            param_frame,
            textvariable=self.param_var,
            values=self.available_params,
            state="readonly",
        ).grid(row=0, column=1, padx=5, pady=5)

        tk.Label(param_frame, text="Values (comma-separated):").grid(
        ...
        tk.Button(self.root, text="Run Sweep and Plot", command=self.run_sweep).grid(
    ...
    def run_sweep(self):
        try:
            ...
            temp_dir = os.path.join(self.project_root, self.config["temp_dir"])
            ...
            messagebox.showerror(
                "Error",
                f"Sweep failed: {str(e)}. Check console for available parameters.",
            )
            ...
            for param_val in param_values:
                ...
                for i in range(0, len(param_columns), curves_per_plot):
                    ...
                    for idx, column in enumerate(
                        param_columns[i : i + curves_per_plot]
                    ):
...
if __name__ == "__main__":
    # Initialize logging system for standalone script execution
    from src.manager.log_manager import log_manager
