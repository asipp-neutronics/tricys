...
from tricys.simulation_plus import run_co_simulation_workflow, run_simulation
...
def test_workflow_single_job(setup_and_teardown, request):
    """Tests the core workflow function for a single job."""
    test_output_dir = setup_and_teardown  # The fixture yields the unique dir path
    config = get_base_config(test_output_dir)
    config["run_timestamp"] = datetime.now().strftime("%Y%m%d_%H%M%S")
    job_params = {}
    job_id = 1

    result_path = run_co_simulation_workflow(config, job_params, job_id)

    assert (
        result_path and Path(result_path).is_file()
    ), "Workflow should return a valid result file path"
    request.node.test_passed = True


def test_integration_single_run(setup_and_teardown, request):
    """
    ...
    result_files = list(run_results_dir.glob("co_simulation_result*.csv"))
    assert len(result_files) == 1, "Expected one final result file"

    request.node.test_passed = True


def test_integration_batch_run(setup_and_teardown, request):
    """
    Tests that a batch (sweep) simulation runs and merges results correctly.
    This test keeps intermediate files for inspection.
    """
    test_output_dir = setup_and_teardown
    config = get_base_config(test_output_dir)
    config["simulation_parameters"] = {"blanket.TBR": [1.05, 1.10]}
    config["simulation"]["concurrent"] = True
