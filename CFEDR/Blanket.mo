...
model Blanket
  ...
  Modelica.Blocks.Interfaces.RealOutput to_CP[5] "输出到Coolant_Loop系统" annotation(
  ...
  parameter Real to_CP_Fraction = 0.01 "输出到CP的比例";
  parameter Real to_TES_Fraction = 1 - to_CP_Fraction "输出到TES的比例";
...
equation
  ...
  for i in 1:5 loop
    ...
    to_CP[i] = to_CP_Fraction * outflow[i];
  end for;
