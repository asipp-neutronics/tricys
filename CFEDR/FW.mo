...
model FW
  ...
  Modelica.Blocks.Interfaces.RealInput from_CP[5] annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput to_CP[5] annotation(
...
equation
  // 计算每种物质的动态变化和输出
  for i in 1:5 loop
    der(I[i]) = from_plasma[i] + from_CP[i] + from_CPS[i] - (1 + nonradio_loss[i]) * I[i] / T  - decay_loss[i] * I[i];
    outflow[i] = I[i] / T;
    to_CP[i] = outflow[i];
  end for;
