    ...
    try:
        ...
        for param_A_val in param_A_vals:
            for param_B_val in param_B_vals:
                logger.info(f"Simulating: {param_A_name}={param_A_val}, {param_B_name}={param_B_val}")
                ...
                output_csv = os.path.join(temp_dir, get_unique_filename(temp_dir, base_filename))
                ...
                if not os.path.exists(output_csv):
                    raise FileNotFoundError(f"Simulation output file not created: {output_csv}")
                counter += 1
            ...
            for param_B_val in param_B_vals:
                ...
                if os.path.exists(csv_file):
                    ...
                    if 'time' not in df or 'sds.I[1]' not in df:
                        logger.warning(f"Invalid CSV format in {csv_file}: {df.columns}")
                        continue
        ...
        combined_csv_path = os.path.join(temp_dir, get_unique_filename(temp_dir, base_combined_filename))
        combined_df.to_csv(combined_csv_path, index=False)
        logger.info(f"Combined CSV file saved to: {combined_csv_path}")

        rises_csv_path = os.path.join(temp_dir, get_unique_filename(temp_dir, "rises_info.csv"))
...
class SimulationUI:
    def __init__(self, root):
        ...
        self.package_path = "D:/FusionSimulationProgram/FFCAS_v0_FusionFuelCycleAnalysisSystem/FFCAS/package.mo"
        self.available_params = get_available_parameters(self.package_path, "FFCAS.Cycle")
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        param_frame = ttk.LabelFrame(self.root, text="Parameter Selection", padding=10)
        ...
        self.param_var = tk.StringVar(value="i_iss.T" if "i_iss.T" in self.available_params else (self.available_params[0] if self.available_params else ""))
        ttk.Combobox(param_frame, textvariable=self.param_var, values=self.available_params, state="readonly").grid(row=0, column=1, padx=5, pady=5)

        tk.Label(param_frame, text="Values (comma-separated):").grid(row=1, column=0, padx=5, pady=5)
        self.param_values_var = tk.StringVar(value="12.0")
        tk.Entry(param_frame, textvariable=self.param_values_var).grid(row=1, column=1, padx=5, pady=5)

        tk.Label(param_frame, text="Tip: Check console for parameters").grid(row=2, column=0, columnspan=2, padx=5, pady=5)
        ...
        tk.Label(self.root, text="Number of Intervals:").grid(row=3, column=0, padx=5, pady=5)
        self.num_intervals_var = tk.IntVar(value=5000)
        tk.Entry(self.root, textvariable=self.num_intervals_var).grid(row=3, column=1, padx=5, pady=5)

        tk.Button(self.root, text="Run Sweep and Plot", command=self.run_sweep).grid(row=4, column=1, pady=10)
    ...
    def run_sweep(self):
        try:
            ...
            try:
                ...
                raise ValueError("Parameter values must be comma-separated numbers (e.g., 12.0 or 6.0,12.0).")
            ...
            logger.info(f"Starting sweep: {param_name}={param_values}, TBR range=[1.01, 1.25]")
            ...
            plot_paths = self.plot_results(result_path, param_name, param_values, stop_time, temp_dir)
            ...
            messagebox.showerror("Error", f"Sweep failed: {str(e)}. Check console for available parameters.")

    def plot_results(self, csv_path, param_name, param_values, stop_time, temp_dir):
        try:
            df = pd.read_csv(csv_path)
            logger.info(f"CSV loaded: {csv_path}, shape={df.shape}, columns={df.columns.tolist()}")
            ...
            for column in df.columns[1:]:
                ...
                if len(data) < 3:
                    logger.warning(f"Skipping column {column}: insufficient data")
                    continue
                diffs = np.diff(data)
                for i in range(1, len(diffs)):
                    if i > len(diffs)//2 and diffs[i] > 0:
                        ...
                        logger.info(f"Rise detected for {column} at {rise_time:.2f}s")
                        break
            ...
            for param_val in param_values:
                param_columns = [col for col in df.columns[1:] if f"{param_name}={param_val:.3f}" in col]
                for i in range(0, len(param_columns), curves_per_plot):
                    plt.figure(figsize=(10, 6))
                    colors = sns.color_palette("tab20", min(curves_per_plot, len(param_columns) - i))
                    for idx, column in enumerate(param_columns[i:i+curves_per_plot]):
                        tbr_value = float(column.split("blanket.TBR=")[1])
                        plt.plot(time, df[column], label=f"TBR={tbr_value:.2f}", color=colors[idx], linewidth=1.0)
                    
                    plt.xlabel("Time (s)")
                    plt.ylabel("sds.I[1]")
                    plt.title(f"sds.I[1] vs Time for {param_name}={param_val:.2f} (Group {i//curves_per_plot + 1})")
                    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=6)
                    plt.grid(True)
                    plt.tight_layout()

                    safe_param_name = param_name.replace('.', '_')
                    png_path = os.path.join(temp_dir, get_unique_filename(temp_dir, f"sds_I1_sweep_{safe_param_name}_{param_val:.2f}_group_{i//curves_per_plot}.png"))
