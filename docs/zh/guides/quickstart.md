...
- 程序会自动将示例文件复制到**当前工作目录**下的 `test_example` 文件夹中，并在该目录中执行任务。
- 这种设计可以确保原始示例文件不被修改，并保持工作区整洁。

!!! tip "Enhanced 模式"
    默认情况下，示例运行器会自动启用 **Enhanced 模式** (Compile Once, Run Many)。这对于大规模参数扫描任务来说，能显著减少编译时间并提高执行效率。如果您在命令行手动运行任务，可以通过追加 `--enhanced` 参数来获得同样的效果。
...
!!! failure "即将弃用"
    `tricys gui` 功能后续将被弃用，建议直接使用命令行工具或 `example` 运行器。
...
| `tricys hdf5` | 启动 HDF5 结果可视化工具，用于高效查看和分析大规模仿真生成的数据文件 (`.h5`)。 |
